...
const opts = {
    ...
    ySearch(searchQuery, opts, function (err, results) {
        if (err) {
            msg.channel.send(`
            Désolé, je n'ai pas trouvé de chanson correspondante.`);
            return console.log(err);
        }
        for (var y = 0; results[y].kind === 'youtube#channel'; y++);
        ytdl.getInfo(results[y].link, function (err, song) {
            if (err) {
                msg.channel.send(`Désolé, je n'ai pas trouvé de chanson correspondante.`);
                return console.log(err);
            }
            const author  = msg.author.username + '#' + msg.author.discriminator;
            guild.queueSong(new Song(song.title, song.video_url, 'youtube',  author, time(song.length_seconds), song.iurlmq));
                    msg.channel.send(
            `Rajouter a la liste d'attente ${tool.wrap(song.title.trim())} (\`${time(song.length_seconds)}\`) demandé par ${tool.wrap(author)}`
        );
    ...
    */
    song(msg, guild, url) {
        ytdl.getInfo(url, (err, song) => {
            if (err) {
                console.log(err);
                msg.channel.send(`Désolé, je ne peux pas mettre votre chanson dans la queud.`);
                return;
            }
            const author  = msg.author.username + '#' + msg.author.discriminator;
            console.log(song);
    ...
    */
    playlist(msg, guild, playlistId) {
        ...
        async function addToQueue(playlistTitle, playlistItems) {
            ...
            msg.channel.send(
                `Rajouté a la liste d'attente ${tool.wrap(playlistItems.length)} songs from ${tool.wrap(playlistTitle)} Demandé par ${tool.wrap(msg.author.username + '#' + msg.author.discriminator)}`
            );
    ...
    for (let guildId in guilds) {
        ...
        if (guild.inactivityTimer <= 0) {
            ...
            guild.musicChannel.send(
                ':no_entry_sign: Je quitte le canal vocal en raison d\'inactivité');
